...
public class ResponseUtils {
    ...
    public static String getHeaderJava8Way(CloseableHttpResponse response, String headerName){
        ...
        //Take list of headers, then stream them
        Header matchHeader = httpHeaders.stream()
                //Filter the header we need
                .filter(header -> headerName.equalsIgnoreCase(header.getName()))
                //Invoke find first, find and return the first header that match the filter criteria, else throw exception
